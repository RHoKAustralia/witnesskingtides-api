...
function streamToFlickr(photoId, payloadData, user) {
  ...
  api(opts, function(err, data) {
    if (err) {
      winston.error('Could not upload photo: ' + err);
      return;
    }
    ...
    api({
      ...
      Photo.findOne({
        ...
        if (err) {
          winston.error('Could not find uploaded photo in FlickR.');
          return;
        }
        ...
        api({
          ...
          if (err) {
            winston.error('Could not set GeoLocation for photo.');
            return;
          }

          winston.info('Photo geotagged successfully!');
        });
      });
    });
